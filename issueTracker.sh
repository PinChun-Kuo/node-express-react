...
docker run --name issue-postgres -e POSTGRES_PASSWORD=12345678 -d postgres
...
docker run -P --name issue_tracker -p 3000:3000 --link issue-postgres:postgres -e POSTGRES_DB=issueTracker -d chloe/issue_tracker
